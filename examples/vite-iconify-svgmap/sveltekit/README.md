...
`vite.config.ts` adds the sveltekit plugin from `@stephansama/vite-iconify-svgmap/sveltekit` after `sveltekit()` so sprites for icons rendered with `getIcon` are written after prerendering
